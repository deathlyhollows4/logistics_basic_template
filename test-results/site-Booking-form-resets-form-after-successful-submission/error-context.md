...
- Name: site.spec.ts >> Booking form >> resets form after successful submission
- Location: tests\site.spec.ts:58:3
...
Call log:
  ...
  - waiting for locator('input[name="name"]')
    14 × locator resolved to <input required="" type="text" name="name" placeholder="Name" value="Reset Test" class="w-full rounded-lg border border-slate-300 p-3"/>
       - unexpected value "Reset Test"

```

```yaml
- textbox "Name": Reset Test
```
...
```ts
  ...
  2   | import { generateAdminCookie } from "./utils";
  3   | 
  4   | async function loginAsAdmin(page: Parameters<Parameters<typeof test>[1]>[0]["page"]) {
  5   |   await page.context().addCookies([generateAdminCookie()]);
  6   | }
  7   | 
  8   | // ── Landing page ─────────────────────────────────────────────────────────
